...
contract Trade {
    ...
    event orderLog(bytes16 seller, bytes16 buyer, bytes32 itemList, uint timeStamp);
    ...
    function orderProceed(
        bytes16 seller, bytes16 buyer, bytes32 itemList
    ) public isOwner{
        emit orderLog(seller, buyer, itemList, block.timestamp);
    }
